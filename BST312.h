...
//Used by deleteNode to find largest in left subtree
template<class ItemType>
...
//inserts nodes into BST
//Duplicate values are placed on the right child
template<class ItemType>
...
bool BST_312<ItemType>::isItemInTree(TreeNode* t, const ItemType& item)
{
	...
	if(t->data == item || isItemInTree(t->left, item) || isItemInTree(t->right, item))
		return true;
